...
documentation-website-zh/Writerside/  # 中文文档（目标）
  ├── topics/                         # 中文主题文件
  ├── hi.tree                         # 已翻译的目录结构（独立副本）
  ├── c.list                          # 已翻译的分类列表（独立副本）
  ├── labels.list                     # 已翻译的标签列表（独立副本）
  ├── snippets -> ../.../snippets     # 软链（自动同步）
  ├── images -> ../.../images         # 软链（自动同步）
  ├── resources -> ../.../resources   # 软链（自动同步）
  ├── v.list -> ../.../v.list         # 软链（版本变量，无需翻译）
  └── writerside.cfg -> ../.../writerside.cfg  # 软链（配置一致，无需翻译）
```

**软链文件**（`snippets`、`images`、`resources`、`v.list`、`writerside.cfg`）自动跟随上游，无需手动同步。

**独立副本**（需翻译）：
- `hi.tree`：翻译 `toc-title` 属性
- `c.list`：翻译 `name` 属性
- `labels.list`：翻译标签描述文本
